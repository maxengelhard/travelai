        ...
        def wrapper_wrapper(handler):
            ...
            def wrapper(event, context):
                ...
                if origin is not None:
                    ...
                    headers["Access-Control-Allow-Headers"] = "Accept-Encoding,Content-Encoding,Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,Client-ID,key,secret,session"
                else:
...
def admin_auth(handler):
    ...
    def wrapper(event, context):
        ...
        decoded_token = jwt.decode(token, options={"verify_signature": False})  # Decode the JWT token without verification

        print(decoded_token)
        
        if 'cognito:groups' in decoded_token and 'admin' in decoded_token['cognito:groups']:
            return handler(event, context)
        else:
            return {
                'statusCode': 401,
